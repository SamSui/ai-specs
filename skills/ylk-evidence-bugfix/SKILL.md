...
- 同一工具、SSH、API 或数据库只读调用连续失败或超时两次；
- 目标环境、仓库、分支、数据源、部署模型或权限无法确认；
- 需要在多个技术方案、数据修复范围、环境或发布策略之间由 Owner 决策；
- 发现任务需要修改未授权仓库、公共历史缺陷、存量数据或生产资源；
- 规范、代码、CI、README 与实际环境的事实冲突。

**受阻时 NEVER：**

- ❌ 静默改用其他环境、其他 API、模拟数据或猜测的命令；
- ❌ 隐藏错误、降低验证标准或把未执行的验证写成通过；
- ❌ 自行扩大范围、选择发布环境、写库或处理存量数据；
- ❌ 为了继续排查而读取、回显或传播未获用户提供的凭据。
- ❌ 对用户已提供的完整只读请求擅自改写认证 Header、Cookie、Referer、请求体或其他参数；需要执行时必须原样使用。

**正确流程：**

1. 停止受阻分支，保留已执行命令、脱敏错误和影响范围；
2. 说明已验证事实、失败点、不能继续的原因及可选路径；
3. 属于 Owner 决策时调用 `interview` Skill，并用 `AskUserQuestion` 提问；
4. 获得明确决定后，从该决定对应的步骤继续，不把决定外推到其他仓库或环境。

未列出的异常也按本协议处理。“未被列出”不是继续执行的许可。
...
任一项不能明确回答“否”或“是”时，STOP 并按本协议报告阻塞。
...
- 后端业务分支以 JUnit + Mockito 覆盖正常路径、关键空值/空白、历史值保留、真实 Mapper/下游参数、持久化字段与新建/更新/重试差异。
